        ...
        <main>
          ...
          <section className="relative w-full">
            ...
            <div className="absolute top-[7vw] left-[4vw]">
              <h1 className="text-purple-950 text-3vw italic font-semibold">
                Contact Us
              </h1>
           ...
           <main className="">
            ...
            {timings?.map((item, index) => (
                    <div key={index} className="flex items-center w-full max-w-[90vw] justify-between m-2vw">
                        ...
                        <h1 className="text-purple-950 text-[2.1vw] font-semibold">{item.title}</h1>
                        <section className="mt-0.5vw">
                           {item?.info?.map((item2, index) => (
                            <div key={index} className="flex items-center">
                               <span className="text-blue-950 text-vw"> {item2.icon}</span>
                        ...
                        <section className="grid -ml-vw grid-cols-2 gap-3">
                            {item?.detail?.map((item3, index) => (
                                    <div key={index} className="mt-vw m-vw">
                                        <h2 className="text-purple-950 font-bold text-[1.5vw]"> {item3?.title}</h2>
                                        {item3?.timimngs?.map((item4, index) => (
                                            <div className="mt-vw" key={index}>
                                            <h2 className="text-purple-950 font-bold text-[0.9vw]"> {item4?.day}</h2>
                                            <p className="text-purple-950 font-normal text-[0.9vw]">{item4?.time}</p>
                                            </div>
                                        ))}
                                    </div>
                            ))}
                ...
                {getInTouch?.map((item, index) => (
                    <div className="relative flex justify-between max-w-[80vw] w-full" key={index}>
                        <div className=" w-full max-w-[35vw]">
                            ...
                            <div className="mt-2vw">
                            <Link to="" className="p-vw bg-purple-950 text-vw text-white font-medium text-center">Get in touch</Link>
                            </div>
                        </div>
